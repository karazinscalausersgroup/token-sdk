 ...
 * [TokenPointer], then the token can evolve independently of who owns it. Otherwise, if the [TokenType] is [FixedToken]
 * then the token definition is inlined into the [FungibleToken] state. This state object implements [FungibleState]
 * as the expectation is that it contains [Amount]s of an [IssuedTokenType] which can be split and merged. All
 * [TokenType]s are wrapped with an [IssuedTokenType] class to add the issuer party, this is necessary so that the
 * [FungibleToken] represents an agreement between an issuer of the [IssuedTokenType] and a holder of the
